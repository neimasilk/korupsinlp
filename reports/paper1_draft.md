...
We present CorpusKorupsi, the first large-scale structured corpus of Indonesian corruption court verdicts extracted from the Supreme Court (Mahkamah Agung) public repository. From 557 cassation and case-review decisions spanning 2011–2026, we extract seven structured fields — prison sentence, prosecution demand, state financial loss, region, year, defendant name, and legal articles — using a regex-based pipeline validated against a 20-case golden set (100% vonis accuracy). We identify a previously unreported confounding variable in Indonesian sentencing research: **pemohon kasasi** (appeal filer identity), finding that while 55% of cases are prosecutor-filed and 34% defendant-filed, a Welch's t-test reveals **no significant difference** in sentencing outcomes (p=0.95, Cohen's d=0.01). We document a strong monotonic relationship between state financial loss and sentence severity (Spearman ρ=0.56, p<10⁻²²), with each log-order increase in kerugian negara corresponding to ~1.41 additional years of imprisonment (R²=0.33). We also identify 17 acquittal cases (5.0%) where the MA overturned convictions. Our corpus, extraction pipeline, and golden set are released to support computational legal studies in the Indonesian context.
...
3. **Methodological finding**: Pemohon kasasi as a confounding variable — while appeal filer identity (prosecutor vs defendant) shapes selection into the dataset, Welch's t-test shows **no significant sentencing difference** (p=0.95, d=0.01), definitively contradicting naive expectations. This must be controlled for in any MA sentencing analysis.
4. **Descriptive findings**: First computational census of sentencing patterns including a strong kerugian-vonis correlation (ρ=0.56, R²=0.33), geographic distribution across 40+ regions, and identification of MA acquittals (5.0%)
...
Linear regression: vonis (years) = 1.41 × log₁₀(kerugian) − 8.28, R²=0.33. Each order-of-magnitude increase in state loss corresponds to ~1.41 additional years of imprisonment.
...
### 6.7 Multivariate Regression (n=254)
To assess which factors independently predict sentence severity, we fit nested OLS models on cases with valid kerugian (n=254):

| Model | Predictors | R² | adj R² | AIC | BIC |
|-------|-----------|-----|--------|-----|-----|
| M1 | log₁₀(kerugian) | 0.334 | 0.332 | 1230 | 1237 |
| M2 | + pemohon_kasasi | 0.335 | 0.329 | 1232 | 1243 |
| M3 | + Jakarta Pusat | 0.338 | 0.330 | 1233 | 1247 |
| M4 | + year | 0.339 | 0.328 | 1234 | 1252 |

**Key result**: Only log₁₀(kerugian) is significant (β=1.33, p<0.001). Pemohon kasasi (p=0.77), Jakarta Pusat (p=0.23), and year (p=0.65) are all non-significant after controlling for state loss magnitude. An F-test comparing M1 to M4 confirms the additional variables do not improve fit (F=0.54, p=0.65). The parsimonious bivariate model (M1) is preferred by BIC.

Quantile regression (median) confirms the OLS pattern: only log₁₀(kerugian) is significant (β=1.42, p<0.001), with all other predictors non-significant. 10-fold cross-validation yields mean R²=0.25 (±0.26) and mean MAE=2.09 years, indicating moderate but unstable predictive power — consistent with substantial unexplained variance from factors not in our model (e.g., pasal charged, defendant's role, cooperation with authorities).

**Interpretation**: State financial loss is the dominant, and effectively sole, sentencing predictor at the MA level. The "Jakarta Pusat effect" and any apparent temporal trend are composition effects driven by the correlation between region/year and kerugian magnitude. This suggests MA sentencing, at least for the variables we can extract, follows a relatively simple proportionality principle: larger losses receive longer sentences, with each log-order increase in kerugian adding ~1.4 years.
...
1. **Selection bias**: Only MA cassation/PK cases (appealed), not first-instance PN verdicts
2. **Pemohon kasasi confound**: 60/36 JPU/terdakwa split with 4% unknown; must stratify in analysis
3. **Extraction errors**: Regex-based. While golden set shows 100% vonis accuracy on 20 diverse cases, systematic failures remain possible on untested patterns
4. **Temporal skew**: 2025 overrepresented (136/327, 42%) due to MA publication patterns and scraping recency — substantially mitigated by year-filtered scraping (2014–2024) with 14 years now represented
5. **Kerugian extraction**: Environmental damage sometimes included, inflating figures (e.g., PT Timah Rp300T includes Rp271T environmental). ~34% of cases have no kerugian extracted — many are gratification (suap) cases where no state loss figure applies
6. **Corpus size**: 557 verdicts (327 with valid vonis) — adequate for descriptive and inferential statistics, and sufficient for multivariate regression with 5+ predictors
7. **Factor extraction limited**: Only ~6% of MA kasasi verdicts contain explicit aggravating/mitigating factor lists (structural limitation of cassation vs first-instance)
8. **Acquittals**: 17 cases (5.0%) now correctly detected, but may be undercount if some acquittals use non-standard phrasing
9. **Empty scrapes**: ~11% of scraped URLs returned error/captcha pages from the MA server, yielding no case data
...
CorpusKorupsi provides the first structured, machine-readable corpus of Indonesian Supreme Court corruption verdicts. Our three key findings are: (1) pemohon kasasi identity, while shaping selection into the dataset, does **not** predict sentence severity (p=0.95, d=0.01) — contradicting naive expectations and requiring methodological awareness in future MA sentencing research; (2) state financial loss (kerugian negara) is the dominant and effectively sole predictor of sentence severity (ρ=0.56, R²=0.33), with each log-order increase corresponding to ~1.4 additional years; and (3) apparent geographic and temporal effects (e.g., harsher Jakarta Pusat sentences) are composition effects driven by kerugian magnitude, disappearing in multivariate analysis. These findings provide the first quantitative evidence base for understanding corruption sentencing patterns in Indonesia.

Compared to ICW's annual sentencing reports (which are descriptive and manual), our approach offers reproducibility, structured data, and confound-controlled analysis. Our mean sentence of 4.63 years (MA cassation) is notably higher than ICW's 2024 finding of 3.25 years across all court levels, consistent with the selection bias expected from cassation cases.

**Future work**:
- **Paper 2**: Extended predictive sentencing model incorporating pasal (articles charged), defendant role, and cooperation indicators via NLP extraction
- **PN Tipikor expansion**: First-instance verdicts with full pertimbangan hakim (judicial reasoning) for factor analysis
...
- **Temporal dynamics**: Causal analysis of whether corruption sentences are genuinely increasing over time, controlling for case composition
