...
Computational sentencing analysis typically relies on bag-of-words (TF-IDF) or neural text representations to extract predictive features from judicial text. We show that for small legal corpora, these approaches fail catastrophically. Using 331 Indonesian corruption verdicts with extracted judicial reasoning (pertimbangan hakim), we conduct a systematic three-phase experiment: (1) 30 TF-IDF experiments across hyperparameters, models, and preprocessing — all significantly hurt prediction in cross-validation (p<0.001); (2) 16 domain-specific keyword features achieve parity with a tuntutan-only baseline; (3) exhaustive feature selection reveals that just **two binary keywords** — whether the judge mentions Pasal 2 (enrichment charges) and gratifikasi (bribery) — constitute the optimal model, outperforming 100 TF-IDF features (p<0.001) and 384-dimensional transformer embeddings. We show that Pasal 2 captures a legally meaningful distinction: after controlling for prosecution demand, Pasal 2 cases receive sentences 0.50 years heavier than Pasal 3 cases (d=0.46, p<0.001). Text-derived charge type outperforms structured metadata because pertimbangan text captures the *operative* charge while metadata lists all charges including alternatives. We further find that the sentencing discount (vonis/tuntutan ratio) is entirely unpredictable from any text feature (R2=-0.06), while judge identity and geographic location show significant but non-predictive effects. Our findings demonstrate that for small legal corpora (<500 documents), domain knowledge encoded as structured features systematically outperforms statistical text representations — a practical guide for legal NLP in low-resource settings.
...
5x10-fold repeated CV: R2=0.528, improvement +0.026 over baseline (p=0.065, 25/50 folds positive).
